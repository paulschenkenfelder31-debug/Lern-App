...
test('Backup rejects malformed history and safely normalizes comfort settings',()=>{assert.throws(()=>C.validateBackup({}));assert.throws(()=>C.validateBackup({schema:1,settings:{modules:[1]},attempts:[{id:1,ms:-1}],sessions:[]}));const b=C.validateBackup({schema:1,settings:{modules:[1],theme:'unknown'},attempts:[],sessions:[],active:{bad:true}});assert.equal(b.active,null);assert.equal(b.settings.theme,'green');assert.equal(b.settings.sessionSize,20);assert.equal(b.settings.keepAwake,true);assert.equal(b.settings.haptics,true);const blue=C.validateBackup({schema:1,settings:{modules:[1],theme:'blue',sessionSize:10,largeText:1,reduceMotion:1,keepAwake:false,haptics:false},attempts:[],sessions:[]});assert.equal(blue.settings.theme,'blue');assert.equal(blue.settings.sessionSize,10);assert.equal(blue.settings.largeText,true);assert.equal(blue.settings.reduceMotion,true);assert.equal(blue.settings.keepAwake,false);assert.equal(blue.settings.haptics,false);});
